...
Token LexicalAnalyzer::KeyWord(ifstream& in, int num, char elem)
{
    ...
    if (count_word_letters == count_all_letters && (!flag))
    {
        if (num == 0 || num == 2)
        {
            return Token(TokenType::TYPE, lexeme, 0);
        }
        else if (flag_to)
        {
            return Token(TokenType::FUNC, lexeme, 0);
        }
        else
        {
...
Token LexicalAnalyzer::processNumber(char elem)
{
    ...
    if(flag_double && !flag_abc)
        return Token(TokenType::DOUBLE_CONST, lexeme, 0);
    else if(!flag_abc)
        return Token(TokenType::INT_CONST, lexeme, 0);
    else
        return Token(TokenType::ERROR, lexeme, 0);
}
